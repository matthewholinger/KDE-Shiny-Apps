...
helpLabel <- function(label, helpText) {
  tagList(
    span(label),
    tags$span(
      "?",
...
ui <- fluidPage(
  ...
  div(
    ...
    tags$h1(
      "ks: Kernel Density Estimation Shiny Application", 
      style = 
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      h4("Upload your Excel file"),
      fileInput(inputId = "file", label = "Upload Excel file", multiple = TRUE),
      ...
      conditionalPanel(
        condition = "input.noise_checkbox == true",
      ...
      tags$a(
        href = "https://cran.r-project.org/web/packages/ks/index.html",
        "For more information about kernel smoothing, click here.",
        target = "_blank"
      ),
    ...
    mainPanel(
      ...
      progressBar(id = "pb", value = 0, title = "KDE Progress:", display_pct = TRUE, total = 100),
...
server <- function(input, output, session) {
  ...
  prepData <- function(raw, name, nameCol, xCol, yCol, zCol, zIncr, habitatDepth, ifNoise, if2D, ifManateeJitter) { # Transforms the data into a usable form
    data <- raw[raw[nameCol] == name,] # Select only rows corresponding to desired animal
    
    if(if2D) {
      data <- select(data, all_of(c(xCol, yCol)))
      colnames(data) <- c("X", "Y")
    } else {
      data <- select(data, all_of(c(xCol, yCol, zCol))) # Select coordinate columns as X,Y,Z
    ...
    if(ifNoise & !if2D) {
      ...
      if(ifManateeJitter) {
        ...
        shallowMin <- habitatDepth * 0.75
        middleMin <- habitatDepth * 0.25
        floorLevel <- 0
        
        data$Z <- sapply(data$Z, function(z) {
          
          # Surface: do not jitter
          if(z >= habitatDepth) {
            return(z)
          }
          ...
          # Shallow zone: top quarter
          if(z > shallowMin) {
            return(runif(1, shallowMin, habitatDepth))
          }
          
          # Middle zone: middle half
          if(z > middleMin) {
            return(runif(1, middleMin, shallowMin))
          }
          
          # Deep zone: bottom quarter
          return(runif(1, floorLevel, middleMin))
        })
        
      } else {
        data$Z <- data$Z + runif(nrow(data), -zIncr, 0)
      }
      
      print(summary(data$Z))
    }
    return(data)
  }
  run <- function(path, sheet, nameCol, xCol, yCol, zCol, dir, out_file, excluded, zIncr, habitatDepth, ifNoise, ifManateeJitter, ifSingle, ifDouble, if2D, percs, ms, ns, pilots, colorSingle, colorDouble1, colorDouble2, opacitySingle, opacityDouble1, opacityDouble2, display2D) { # Runs program
    ...
    if(ifSingle) {
      ...
      for(i in 1:nrow(names)) {
        ...
        data <- prepData(raw, name, nameCol, xCol, yCol, zCol, zIncr, habitatDepth, ifNoise, if2D, ifManateeJitter) # Preprocess data
  ...
  output$scaling_slider <- renderUI({
    ...
    tagList(
      sliderInput(
        "scaling_slider_server",
        helpLabel("Scaling Factor (m)", "Controls the scaling applied to the KDE bandwidth."),
        1, 10, 1, 1
      ),
      div(
        "Controls the scaling applied to the KDE bandwidth. Larger values produce smoother, broader contours.",
        style = "font-size:12px; color:#666; margin-top:-8px; margin-bottom:10px;"
      )
    )
  })
  ...
  output$stages_slider <- renderUI({
    ...
    tagList(
      sliderInput(
        "stages_slider_server",
        helpLabel("Number of Stages in Bandwidth Optimization", "Sets how many optimization stages are used when estimating KDE bandwidth."),
        1, 10, 1, 1
      ),
      div(
        "Sets how many optimization stages are used when estimating KDE bandwidth.",
        style = "font-size:12px; color:#666; margin-top:-8px; margin-bottom:10px;"
      )
    )
  })
  ...
  output$samse <- renderUI({
    ...
    checkboxInput(
      "samse_checkbox",
      helpLabel("samse", "A KDE bandwidth selection method. This is often a good default starting option."),
      value = FALSE
    )
  })
  output$unconstr <- renderUI({
    ...
    checkboxInput(
      "unconstr_checkbox",
      helpLabel("unconstr", "An unconstrained bandwidth selection method for KDE."),
      value = FALSE
    )
  })
  output$dscalar <- renderUI({
    ...
    checkboxInput(
      "dscalar_checkbox",
      helpLabel("dscalar", "A scalar bandwidth option that applies the same scale more uniformly across dimensions."),
      value = FALSE
    )
  })
  output$dunconstr <- renderUI({
    ...
    checkboxInput(
      "dunconstr_checkbox",
      helpLabel("dunconstr", "A diagonal unconstrained bandwidth option for KDE."),
      value = FALSE
    )
  })
  ...
  output$noise <- renderUI({
    ...
    tagList(
      checkboxInput(
        "noise_checkbox",
        helpLabel("Add manatee depth jitter", "Uses Habitat Depth to create shallow, middle, and deep zones. Surface and floor points are not jittered."),
        value = TRUE
      ),
      div(
        "Adds jitter to depth values in 3D data so observations are spread within allowed depth ranges instead of stacking at one exact depth.",
        style = "font-size:12px; color:#666; margin-top:-8px; margin-bottom:10px;"
      )
    )
  })
  output$manatee_jitter <- renderUI({
    req(input$file)
    req(!is.null(input$twoD_checkbox))
    req(!(input$twoD_checkbox))
    req(!is.null(input$noise_checkbox))
    req(input$noise_checkbox)
    
    tagList(
      div(
        "Manatee jitter uses Habitat Depth to create three zones: shallow top quarter, middle half, and deep bottom quarter. Surface and floor points are not jittered.",
        style = "font-size:12px; color:#666; margin-top:-8px; margin-bottom:10px;"
      )
    )
  })
  ...
  output$enclosure_depth <- renderUI({
    ...
    tagList(
      numericInput(
        "enclosure_depth_input",
        helpLabel("Habitat Depth in Meters", "The total water depth used to define vertical depth zones."),
        1, 1, 1000, 1
      ),
      div(
        "The total water depth used to define vertical depth zones if you checked to add noise.",
        style = "font-size:12px; color:#666; margin-top:-8px; margin-bottom:10px;"
      )
    )
  })
  ...
  output$contours <- renderUI({
    ...
    tagList(
      textInput(
        "contours_input",
        helpLabel("Input Contours", "Enter contour percentages separated by commas, such as 50,95."),
        "50"
      ),
      div(
        "Enter contour percentages separated by commas, such as 50,95. Lower contours show core-use areas and higher contours show broader-use areas.",
        style = "font-size:12px; color:#666; margin-top:-8px; margin-bottom:10px;"
      )
    )
  })
  ...
  output$animal1_color1 <- renderUI({
    req(input$file)
    selectInput("a1c1", label = "Animal 1 Contour Color 1", choices = COLORS, selected = "red")
    })
  output$animal1_color2 <- renderUI({
    req(input$file)
    selectInput("a1c2", label = "Animal 1 Contour Color 2", choices = COLORS, selected = "orange") 
  })
  ...
  output$animal2_color1 <- renderUI({
    ...
    selectInput("a2c1", label = "Animal 2 Contour Color 1", choices = COLORS, selected = "green")
  })
  output$animal2_color2 <- renderUI({
    req(input$file)
    req(!is.null(input$double_trial_checkbox))
    req((input$double_trial_checkbox))
    selectInput("a2c2", label = "Animal 2 Contour Color 2", choices = COLORS, selected = "blue")
  })
  ...
  observeEvent(input$action1, {
    ...
    run(path, sheet, nameCol, xCol, yCol, zCol, dir, out_file, excluded, zIncr, habitatDepth, ifNoise, ifManateeJitter, ifSingle, ifDouble, if2D, percs, ms, ns, pilots, colorSingle, colorDouble1, colorDouble2, opacitySingle, opacityDouble1, opacityDouble2, display2D)
