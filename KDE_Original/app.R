...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      h4("Is your data two dimensional, or three dimensional?"),
      ...
      conditionalPanel(
        condition = "input.twoD_checkbox == false && input.noise_checkbox == true",
...
server <- function(input, output, session) {
  ...
  output$twoD <- renderUI({
    req(input$file)
    checkboxInput("twoD_checkbox", "Check if data is two dimensional, otherwise the default is three dimensional", value = FALSE)
  })
  output$noise <- renderUI({
    ...
    tagList(
      checkboxInput(
        "noise_checkbox",
        helpLabel("Check to Add Noise to three dimensional data", "Adds jitter to depth values in 3D data."),
        value = FALSE
      ),
      div(
  ...
  output$depth_sections <- renderUI({
    ...
    tagList(
      numericInput(
        "depth_sections_input",
        helpLabel("Number of Depth Sections", "Sets how many vertical depth zones are used when defining depth-based jitter."),
        1, 1, 1000, 1
      ),
      div(
        "Sets how many vertical depth zones are used when defining depth-based jitter.",
        style = "font-size:12px; color:#666; margin-top:-8px; margin-bottom:10px;"
      )
    )
  })
